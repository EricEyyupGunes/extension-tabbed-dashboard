...
export const EmbedDashboard: React.FC<EmbedProps> = ({
  ...
  const prepareWorksheet = (
    ...
    new Promise((resolve, reject) => {
      sdk
        .ok(
          sdk.run_query({ query_id: element.query_id, result_format: 'json' })
        )
        .then((query_data: any) => {
          const ws = XLSX.utils.json_to_sheet(query_data)
  ...
  const getDashboard = async (sdk: any, dashboards: any) => {
    ...
    const wb = await Promise.all(dashboards.map(createWorkBook))
    const workbook = {
      SheetNames: wb.map(b => b.SheetNames).reduce((a,b)=> ([...a, ...b]), []),
      Sheets: wb.map(s => s.Sheets).reduce((a,b)=> 
        ({...a, ...Object.keys(b)
          .reduce((x, y) => ({...x, ...b[y]}), {})}
          ), 
          {})
